...
//				$res1 = Execute("update tbl_match_profile set deleted = '1' where userid = '" . $id . "' and matchid = '$matchid'");
		...
		function InsertMatchingPartner($UserId) {
		
			$user = GetSingleRecord("tbl_register","id",$UserId);
			
			$sql = "select id from tbl_register where 1 = 1 and id != '" . $user[id] . "' and enable = 1 and verifiedStatus = 1 and hideProfile = 0 and deleteProfile = 0 and gender != '" . $user[gender] . "' ";
			
			$sql1 = "update tbl_register set ";
			
			
		
			if ($user[religion]) {
					$sql .= "and religion in (" . $user[religion] .") ";
					$sql1.="partnerReligion='". $user[religion]."'";					
			}	
			
			if ($user[caste]) {
					$sql .= "and caste in (" . $user[caste] .") ";		
					$sql1.=",partnerCaste='". $user[caste]."'";
			}
			if($user[gender]=='M') {
				$sql .= "and age <= '" . $user[age] . "'";
				$sql1 .= ",ageFrom = '18'";
				$sql1 .= ",ageTo = '" . $user[age] . "' ";
			}
			if($user[gender]=='F') {
				$UserTo=$user[age]+10;
				$sql .= "and age >= '" . $user[age] . "' and age <= '" . $UserTo . "' ";
				$sql1 .= ",ageFrom = '" . $user[age] . "'";
				$sql1 .= ",ageTo = '" . $UserTo . "' ";
			}
			$sql1.="where id = '" . $user[id] . "'";
