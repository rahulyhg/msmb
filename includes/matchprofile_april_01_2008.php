<?php
	class PartnerMatch {
		function InsertMatch($id) {					
			...
			$exist = Execute("delete from tbl_match_profile where userid = '" . $user[id] . "'");
			...
			if (!$partnerPref_notcompleted) {			
				...
				if ($user[partnerHaveChild] && $user[partnerHaveChild] != 'unspec') {
					...
					if ($partnerHaveChild == "no") {
						$sql .=	"and (no_of_Children = '' or isnull(no_of_Children)) ";	
				...
				if (mysql_num_rows($searchMaxRows) > 0) {
					while ($matchRes = mysql_fetch_array($searchMaxRows)) {
						//$exist = Execute("select * from tbl_match_profile where userid = '" . $user[id] . "' and matchid = '" . $matchRes[id] . "' and deleted='0'");
						//if (mysql_num_rows($exist) > 0) {
							//$exist = Execute("delete from tbl_match_profile where userid = '" . $user[id] . "'  and matchid = '" . $matchRes[id] . "' and deleted='0'");
