...
// can pre-select the right date
		...
		function getreligion() {
			//alert("aaa");
			f1 = document.thisForm;					
			f1.religion.length = 0;
			if (f1.domain.value != "") {
				
				var opt = new Option();						
				opt.value = "";	
				opt.text = "--select religion--";
				f1.religion.options[0] = opt;
				var opt = new Option();						
				opt.value = "";	
				opt.text = "Religion";
				f1.religion.options[1] = opt;
				...
				ajaxRequest.open("GET","includes/fill_domain_religion_ajax.php?action=Getreligion&domainid=" + val, true);
				ajaxRequest.send(null);
			}
		}
