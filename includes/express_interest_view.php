...
$k = 1;	 
while ($exp_interest = mysql_fetch_array($searchRes)) {	
 	
	$member = GetSingleRecord("tbl_register","id",$exp_interest[$type]);	
	if ($member) {	 	
  ...
  <tr>
	...
	<td width="78%" valign="top" bgcolor="#FFFFFF">
		<table width="100%" border="0" cellspacing="5" cellpadding="0" class="btxt">
		  ...
		  <td width="20%" height="20" class="orgin">Recent Login</td>
	...
	<tr>
		<td colspan="2" width="78%" valign="top" bgcolor="#FFFFFF">
		<table width="100%" border="0" cellspacing="5" cellpadding="0" class="btxt">			  
			  <? if ($action == "received" && $action1 != 2 && $action != 3) { ?>
			  <tr>
				<td height="21">&nbsp; Message</td>
				<td>:</td>
				<td>Date Received : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?>&nbsp;&nbsp;</td>
			  </tr>
			  <tr>
				<td height="21" colspan="3"><b><?=$exp_interest[message];?></b></td>		
			  </tr>
			  <? } else if ($action == "received" && $action1 == 2) { ?>
			  <tr>
				<td height="21" colspan="2">&nbsp; Date Received : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?></td>
				<td align="right">&nbsp;&nbsp; Date Accepted : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateAccept]));?></td>
			  </tr>	
			  <? } else if ($action == "received" && $action1 == 3) { ?>
			  <tr>
				<td height="21" colspan="2">&nbsp; Date Received : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?></td>
				<td align="right">&nbsp;&nbsp; Date Rejected : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateReject]));?></td>
			  </tr>
			  <? } ?>
			  <? if ($action == "sent" && $action1 != 2 && $action != 3) { ?>
			  <tr>
				<td height="21" colspan="2">&nbsp; Message :</td>
				<td align="right">Date Sent : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?>&nbsp;&nbsp;</td>
			  </tr>
			  <tr>
				<td height="21" colspan="3"><b><?=$exp_interest[message];?></b></td>		
			  </tr>
			  <? } else if ($action == "sent" && $action1 == 2) { ?>
			  <tr>
				<td height="21" colspan="2">&nbsp; Date Sent : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?></td>
				<td align="right">&nbsp;&nbsp; Date Accepted : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateAccept]));?></td>
			  </tr>	
			  <? } else if ($action == "sent" && $action1 == 3) { ?>
			  <tr>
				<td height="21" colspan="2">&nbsp; Date Sent : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateSent]));?></td>
				<td align="right">&nbsp;&nbsp; Date Rejected : <? echo strftime("%d %b %Y %I:%M %p",strtotime($exp_interest[dateReject]));?></td>
			  </tr>
			  ...
			  <? if ($action == "received" && $action1 != 2 && $action != 3) { ?>
			  <tr>
				<td align="center" colspan="3"><input type="button" name="accept_<?=$member[id]?>" value="Accept" class="button" onclick="funAccept('<?=$member[id]?>');">
					&nbsp;&nbsp;<input type="button" name="reject_<?=$member[id]?>" value="Reject" class="button" onclick="funReject('<?=$member[id]?>');">
				</td>
			  </tr>
			  <? } ?>
			</table>
	...
	<td colspan="2" bgcolor="#f0edd4">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<td width="35" align="center"><a onclick="fnCompare();"  style="cursor:pointer"><img src="images/search_icon_comparin.jpg" title="Compare Profile" border="0"/></a></td>
			...
			<td width="35" align="center"><a href="forward_profile.php?mode=bookmark&id=<?=$member[id]?>"><img src="images/icon_boolin.jpg" border="0" title="Book Mark"/></a></td>						
			<? if ($member[horoscope]) {?>			
			...
			<td width="35" align="center"><img src="images/home_icon.gif" title="View Contact" border="0" onclick="javascript:openContact('address','<?=base64_encode($member[id])?>')" style="cursor:pointer"/></td>								
			<td width="35" align="center"><a href="forward_profile.php?id=<?=$member[id]?>&mode=forward" class="mored"><img src="images/icon_forin.jpg"  title="Forword Profile" border="0"/></a></td>						
			<td>&nbsp;</td>
...
<script language="javascript">
	function funAccept(id) {		
		location.href = "express_interest.php?action=received&mode=accept&mem_id=" + id;
	}
	function funReject(id) {
		if (confirm("Are you sure you want to reject")) {
			location.href = "express_interest.php?action=received&mode=reject&mem_id=" + id;
		}	
	}	
</script>
