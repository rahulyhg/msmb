...
if ($action == "change") {
	...
	$res = Execute("select * from tbl_register where id = '" . $config[userinfo][id] . "' and password = '" . GetVar("oldpassword") . "'");
	if (mysql_num_rows($res) > 0) {
		$res1 = Execute("update tbl_register set password = '" . GetVar("newpassword") . "' where id = '" .$config[userinfo][id]. "'");
		$_SESSION['msg'] = 'Password changed successfully';
		header("Location: thanks.php?id=14");
		die();
	} else {
		$_SESSION['msg'] = 'Invalid Old Password';
		header("Location: error.php?id=15");
		die();
	}
  ...
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
	...
	function passwordValidate() {	
		  ...
		  if (isLen(f1.oldpassword,5,"old Password")){ return false;}
		  if (isNull(f1.newpassword,"new password")) { return false;}
		  if (isLen(f1.newpassword,5,"new Password")){ return false;}
						  ...
						  <tr>
						  	<td valign="top" style="padding-top:35px;">
								<table border="0" cellpadding="0" width="100%" cellspacing="0"  class="reghbtm">																			
						 			...
						 			<tr>
									 	<td colspan="3" align="left"><b class="clr"><?=$_SESSION['msg']?><? $_SESSION['msg'] = ""; ?> </b></td>
									 </tr>
									 <form name="thisForm" method="post" onSubmit="return passwordValidate();">
										<input type="hidden" name="action" value="change">
									 <tr>
										<td width="217" height="30" style="padding-left:30px;">Enter the old password <font color="#FF0000">&nbsp;</font> </td>
									   <td width="353" height="30"><input type="password" name="oldpassword" class="txtbox" maxlength="255"></td>					
									 </tr>
                                     <tr>
