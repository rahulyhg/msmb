...
(function () {
    ...
    function init() {
        // status checks
        if (document.getElementById('PPMiniCart')) {
            return;
        }
    ...
    function createCart() {
        ...
        cart = createElement('form', null, {
            ...
            style: {
                ...
                width: '365px',
                minHeight: '155px',
                background: '#fff url(' + MiniCart.base + window.staticurl +'images/minicart/paypal_logo.gif) no-repeat 10px 15px',
        ...
        actionButton = createElement('a', 'Checkout', {
            ...
            style: {
                ...
                fontWeight: 'normal',
                color: '#000',
                background: '#ffa822 url(' + MiniCart.base + window.staticurl + 'images/minicart/button_submit.gif) repeat-x left center',
        ...
        // shipping
        var shippingNotice = createElement('span', '', {
            style: {
                display: 'block',
                fontSize: '10px',
                fontWeight: 'normal',
        ...
        summaryInfo = createElement('p', [actionButton, subTotal, shippingNotice], {
            style: {
                margin: '0',
                padding: '0 0 0 20px',
                background: 'url(' + MiniCart.base + window.staticurl + 'images/minicart/arrow_down.gif) no-repeat left center',
                fontWeight: 'bold',
                fontSize: '13px'
            },
    ...
    function showCart() {
        ...
        summaryInfo.style.backgroundImage = 'url(' + MiniCart.base + window.staticurl +'images/minicart/arrow_up.gif)';
    }
    ...
    function hideCart() {
        ...
        summaryInfo.style.backgroundImage = 'url(' + MiniCart.base + window.staticurl + 'images/minicart/arrow_down.gif)';
    }

MiniCart.wy_addToCart = function(data) {
    addToCart(data);
}
    ...
    function addToCart(product) {
        ...
        if (!product.item_name && !product.item_number) {
            var emptyItem = createElement('input', null, {
                type: 'hidden',
                disabled: true
            });
            var emptyLine = createElement('li', emptyItem, {
                style: {
                    display: 'none'
                }
            }, itemList);
    ...
    fullname.push(name);
    name = (name.length > 35) ? name = name.substr(0, 35) + '...' : name;

    var itemName = createElement('a', "", {
        href: "#",
        style: {
            textDecoration: 'none',
            border: '0',
    ...
    var itemOptions = createElement('a', meta, {
        style: {
            display: 'block',
            fontSize: '12px',
            color: '#333',
    ...
    var itemX = createElement('img', null, {
        src: MiniCart.base + window.staticurl +'images/minicart/delete.png',
        style: {
            position: 'absolute',
            right: '63px',
            top: '9px',
...
MiniCart.wy_init = function(data){
    //init();
    //hijackForms();
}

MiniCart.wy_loaddata = function() {
    loadData();
}

MiniCart.wy_showdata = function() {
    return MiniCart.products;
}

MiniCart.wy_savedata = function() {
    saveData();
}
    ...
    var add = function (event) {
        ...
        if (this.cmd.value === '_cart') {
            for (i = 0; i < this.elements.length; i++) {
                if (/^(?:item_number|item_name|amount|discount_amount|quantity|on|os|option_|tax|weight|handling|shipping|new_or_upgrade)/.test(this.elements[i].name)) {
...
function doCheckout(e) {
    ...
    if (MiniCart.products.length > 0) {
        ...
        var form = createElement('form', null, {
            ...
            action: '/paypalipn/index.php?action=processupgradeNew',
            method: 'post',
            style: {
                display: 'none'
            }
...
MiniCart.disableShowCart = function() {
    cart.hide();
}
...
function saveData(duration) {
    ...
    document.cookie = 'upgradeMinicart=' + escape(JSON.stringify(data)) + '; expires=' + date.toGMTString() + '; path=' + MiniCart.cookiePath;
}
