...
if($_REQUEST['Mode']=="upgrade"){
	if($_REQUEST['Chks']!=""){		
		for($iC=0;$iC<count($_REQUEST['Chks']);$iC++){
			$sql="select * from astro_pay where id='".$_REQUEST['Chks'][$iC]."'";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)>0){
				...
				$sql_update="update astro_pay set status='Approved' where id='".$obj->id."'";
...
header("Location:Astro_Payment.php");
die();	
}
...
function fnApprove(){
	...
	document.thisForm.action="Astro_Payment.php?Mode=upgrade";
					...
					<tr><td align="center" valign="top">
						...
						<tr><td>
						   ...
						   <? 
							...
							<? getpageNumbers($pager->numPages,$page,"Astro_Payment.php");?>				
						</td></tr>
