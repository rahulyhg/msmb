...
if($_REQUEST['mode']=="del"){
	if($_REQUEST['ChkS']<>""){
		for($i=0;$i<count($_REQUEST['ChkS']);$i++){			
			$sql_chk="select * from tbl_state_master where id = '" . $_REQUEST['ChkS'][$i] . "'";
			$res_chk=mysql_query($sql_chk);
			if(mysql_num_rows($res_chk)>0){	
				$sql="delete from tbl_state_master where id = '" . $_REQUEST['ChkS'][$i] ."'";
				mysql_query($sql);
				echo mysql_error();			
...
function fnDelete() {
	if(notChecked(document.thisForm.elements["ChkS[]"],"State to delete")) {return;}
	if(confirm("Are you sure to delete the selected State(s)?")){
		document.thisForm.action="view_state.php?mode=del";
			...
			<td>
				...
				<tr class="subtitlebg"><td>
					...
					<tr>
						...
						<td align="right"><a href="add_state.php">Add State</a></td>
					</tr>
					</table>
				</td></tr>
				<!-- End : Sub Title -->
				...
				<!-- Start : Table Of Contents -->
					...
					<tr><td align="center" valign="top">
						...
						<tr><td>
						<?
						$sql="select * from tbl_state_master order by id asc";
						$res=mysql_query($sql);	
						echo mysql_error();
						...
						<?	while($rs=mysql_fetch_object($res)){ ?>
								...
								<tr class="tblContent">
									...
									<td width="250">
									<? $country = GetSingleField("country","tbl_country_master","id",$rs->country);?>
										<? echo $country;?>
									</td>
									<td width="250"><a href="add_state.php?id=<? echo $rs->id;?>"><? echo $rs->state;?></a></td>									
 								</tr>
						<?
								$rCnt++;
							}	?>		
							...
							<? getpageNumbers($pager->numPages,$page,"view_state.php");?>
						<?
							mysql_free_result($res);
						}else{
							echo "<br><br><center>No State(s) details Found.</center>";
						}
						?>
						</td></tr>
						</table>
