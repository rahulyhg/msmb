...
if($_REQUEST['mode']=="del"){
	if($_REQUEST['ChkS']<>""){
		for($i=0;$i<count($_REQUEST['ChkS']);$i++){			
			$sql_chk="select * from tbl_religion_master where id = '" . $_REQUEST['ChkS'][$i] . "'";
			$res_chk=mysql_query($sql_chk);
			if(mysql_num_rows($res_chk)>0){	
				$sql="delete from tbl_religion_master where id = '" . $_REQUEST['ChkS'][$i] ."'";
				mysql_query($sql);
				echo mysql_error();			
		...
		header("location:view_religions.php");
		die();
	}
}	
...
function fnDelete() {
	if(notChecked(document.thisForm.elements["ChkS[]"],"Religion to delete")) {return;}
	if(confirm("Are you sure to delete the selected Religion(s)?")){
		document.thisForm.action="view_religions.php?mode=del";
			...
			<td>
				...
				<tr class="subtitlebg"><td>
					...
					<tr>
						...
						<td align="right"><a href="add_religions.php">Add Religion</a></td>
					</tr>
					</table>
				</td></tr>
				<!-- End : Sub Title -->
				...
				<!-- Start : Table Of Contents -->
					...
					<tr><td align="center" valign="top">
						...
						<tr><td>
						<?
						$sql="select * from tbl_religion_master order by id asc";
						$res=mysql_query($sql);	
						echo mysql_error();
						...
						<?	while($rs=mysql_fetch_object($res)){ ?>
								...
								<tr class="tblContent">
									...
									<td width="250">
									<? $domain = GetSingleField("domain","tbl_domain_master","id",$rs->domain);?>
										<? echo $domain;?>
									</td>
									<td width="250"><a href="add_religions.php?id=<? echo $rs->id;?>"><? echo $rs->religion;?></a></td>									
 								</tr>
						<?
								$rCnt++;
							}	?>		
							...
							<? getpageNumbers($pager->numPages,$page,"view_religions.php");?>
						<?
							mysql_free_result($res);
						}else{
							echo "<br><br><center>No Religion(s) details Found.</center>";
						}
						?>
						</td></tr>
						</table>
