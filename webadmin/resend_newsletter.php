...
#$image_url="http://bestmatrimonial.com//images/logo.jpg";
//$server_url="http://www.24x7a2z.com/Maa Shakti Marriage Bureau/ver2/";
$server_url="http://bestmatrimonial.com//";


if($_REQUEST['mode']=='Updatenewsletter')
{
	
   if($_REQUEST['FCKeditor1']=="")
   {	
		$_SESSION['_msg'] = "Description is empty. Please fill the description.";
		header("location:resend_newsletter.php?newsletter_id=".$_REQUEST['newsletter_id']);
		die();
   }
		
   if($_REQUEST['hiddes']!="")
   {
		$delfile =$_REQUEST['hiddes'];
		unlink("../sendnewsletter_files/$delfile");
   }
   ...
   fclose($fileid);
   mysql_query("update tbl_newsletter set title='".$_REQUEST['txtTitle']."',subject='".$_REQUEST['txtSubject']."',status='".$_REQUEST['ddlSelectCustomer']."',description='".$filename."',created_date=now() where newsletter_id=".$_REQUEST['newsletter_id']."");
   echo mysql_error();
...
if($_REQUEST['mode']=='save')
{
   ...
   {	   		
	...
	$StrFrom   ="noreply@matrimonialclone.com";
	$StrSubject= $_SESSION['txtSubject'];
	$StrContent= stripslashes($_REQUEST["FCKeditor1"]);;
	$subject=$StrSubject;
	//$Strdate =convertdate_1($_SESSION['txtdate']);
				...
				$strEmployeeIDs="";
					...
					$strEmployeeIDs .="'".$obj->id."',";
						...
						# Mail message starts here #		
							...
							$message.="<td>";
								$message.="Dear Subscriber, <br><br>";	
								$message.=$_SESSION['description']."<br><br>";
								$message.="Warm Regards,<br>";
								$message.="Matrmonial shaadi  Team";
								$message.="</td>";							
							$message.="</tr>";	
...
function fnNewsletterupdate(){  
	...
	document.thisForm.action="resend_newsletter.php?mode=Updatenewsletter";
			...
			<td>
				...
				<tr class="subtitlebg"><td>
					...
					<tr>
						...
						<td align="right"><a href="view_subscribers.php">View Subscribers</a>&nbsp;|&nbsp;<a href="send_newsletter.php">View Newsletters</a></td>
 					</tr>
					</table>
				</td></tr>
				<!-- End : Sub Title -->
				...
				<form name="thisForm" method="post"  action="resend_newsletter.php?mode=save" enctype="multipart/form-data" onSubmit="return fnValidate();">
				<input type="hidden" name="sendmail" value="sendmail">
				<input type="hidden" name="newsletter_id" value="<?=$_REQUEST['newsletter_id']?>">
					...
					<tr><td align="center" valign="top">
						...
						<tr><td>
							...
							<tr class="tblContent"><td>
								...
								{
								   $sql="select * from tbl_newsletter where newsletter_id=".$_REQUEST['newsletter_id'];
								   //echo $sql;
						           $res=mysql_query($sql);	
								   ...
								   $filename = "../sendnewsletter_files/$content";
								   $fd = fopen ($filename, "r");
								...
								?>
								<input type="hidden" name="hiddes" value="<?=$obj->description;?>">
								<table cellpadding="5" cellspacing="1" border="0" width="600" class="tblBorder">								
									<tr class="tblContent">
										<td>Title<font color="#FF0000">*</font></td>
										<td><input type="text" name="txtTitle" class="txtbox" id="title" value="<?=$title?>" maxlength="255"></td>
									</tr>
									<tr class="tblContent">
										<td>Subject<font color="#FF0000">*</font></td>
										<td><input type="text" name="txtSubject" class="txtbox" id="author" value="<?=$subject?>" maxlength="255"></td> 
									</tr>
									<tr class="tblContent">
									...
									<script language="javascript">
									document.thisForm.ddlSelectCustomer.value=<?=$status?>
									</script>
									<tr class="tblContent">
									...
									<tr class="tblContent">
										...
										<input type="button" value="Update" class="butten" onClick="fnNewsletterupdate();">
										<input type="submit" value="Resend Newsletters" class="butten">
										</td>
									</tr>
								</table>
							</td></tr>
